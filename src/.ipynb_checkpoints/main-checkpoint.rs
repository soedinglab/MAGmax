...
fn main() -> io::Result<()> {
    ...
    let (graph, ani_details) = match merge::calc_ani(&bindir, &bin_qualities, &format, anifile, ani_cutoff, contamination_cutoff, alignedfrac) {
...
fn process_components(
    ...
    if component.len() == 1 {
        ...
        if let Some(quality) = bin_qualities.get(binname) {
            debug!("quality of binname {:?}", quality);
